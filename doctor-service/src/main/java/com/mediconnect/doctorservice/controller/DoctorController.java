...
public class DoctorController {
    ...
    @PostMapping("/bulk")
    public ResponseEntity<ApiResponse<List<DoctorResponse>>> createDoctorsBulk(
            @RequestBody List<DoctorRequest> requests) {
        return ResponseEntity.status(HttpStatus.CREATED)
    ...
    public ResponseEntity<ApiResponse<List<DoctorResponse>>> getDoctorsByFilters(
        ...
        log.info("Request received in doctor controller at: {}", request.getRequestURI());
        // log.info("parameters: {}", name,sct);
    ...
    @PutMapping("/{doctorId}/education/{educationId}")
    @PreAuthorize("hasRole('DOCTOR')")
    public ResponseEntity<ApiResponse<EducationResponse>> updateEducation(
            ...
            Authentication authentication) {
        UUID userId = UUID.fromString(authentication.getName());
        return ResponseEntity.ok(doctorService.updateEducation(doctorId, educationId, request, userId));
    }

    @PutMapping("/{doctorId}/experience/{experienceId}")
    @PreAuthorize("hasRole('DOCTOR')")
    public ResponseEntity<ApiResponse<ExperienceResponse>> updateExperience(
            ...
            Authentication authentication) {
        UUID userId = UUID.fromString(authentication.getName());
        return ResponseEntity.ok(doctorService.updateExperience(doctorId, experienceId, request, userId));
    }
